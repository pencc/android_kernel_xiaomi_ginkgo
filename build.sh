...
KERN_IMG="/workspace/ryuzenn/out-A11/arch/arm64/boot/Image.gz-dtb"
DTBO_IMG="/workspace/ryuzenn/out-A11/arch/arm64/boot/dtbo.img"
export KBUILD_BUILD_USER="EdwiinKJ"
export KBUILD_BUILD_HOST="RastaMod69"
export PATH="/workspace/ryuzenn/toolchain/rastamod-clang/bin:$PATH"
export LD_LIBRARY_PATH="/workspace/ryuzenn/toolchain/rastamod-clang/lib:$LD_LIBRARY_PATH"
export KBUILD_COMPILER_STRING="$(/workspace/ryuzenn/toolchain/rastamod-clang/bin/clang --version | head -n 1 | perl -pe 's/\((?:http|git).*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//' -e 's/^.*clang/clang/')"
export out=/workspace/ryuzenn/out-A11
